import { Link } from 'react-router-dom'
import getImageURL from '../../utils/getImageURL'
import topThePage from '../../utils/topThePage'
import { TbWriting } from 'react-icons/tb'
import { FaCalendar } from 'react-icons/fa'
import datetimeToString from '../../utils/datetimeToString'


const ArticleTile = ({id, title, cover, type, date}) => {
    const getTileClass = () => {
        const tileClass = 'mb-6 md:mb-20 rounded-2xl text-neutral-200 hover:text-neutral-800 col-span-2 md:col-span-1'
        const colorByType = type !== 'eventNews' ? ' bg-blue-600 hover:bg-blue-400' : ' bg-indigo-500 hover:bg-indigo-300'
        return tileClass + colorByType
    }

    let link

    switch (type) {
        case 'concert':
            link = `/concerts/${id}`
            break
        case 'news':
            link = `/actualites/${id}`
            break
        case 'eventNews':
            link = `/actualites/${id}`
            break
        default:
        	console.error('Error ArticleTile')
            link = '/'
            break
    }

    return (
        <>
            <Link
                to={link}
                onClick={topThePage}
                className={getTileClass()}
            >
                <img className='rounded-t-2xl' src={getImageURL(cover)} alt={'cover image'}  />
                <div className='m-3'>
                    <h2 className='text-3xl font-bold'>{title}</h2>
                    <h5 className='text-base ml-3'>
                        {type === 'news'
                            ? <TbWriting className='inline mr-1' />
                            : <FaCalendar className='inline mr-1 mb-1' />
                        }
                        {datetimeToString(date, 'short', type !== 'news')}
                    </h5>
                </div>
            </Link>
        </>
    )
}

export default ArticleTile